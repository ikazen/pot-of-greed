...
def test_extract_refs_branch_article():
    """#40: 가지번호 조문("제18조의3")도 하나의 조문 인용으로 추출되어야 한다."""
    from app.retrieval.refs import extract_refs
...
def _patch_pool(monkeypatch, articles=frozenset(), cases=frozenset()):
    ...
    import app.retrieval.refs as refs_mod
...
async def test_verify_refs_exist_existing(monkeypatch):
    ...
    from app.retrieval.refs import verify_refs_exist
...
class TestParseRef:
    ...
    def test_article_branch(self):
        """#40: "제18조의3" 가지번호 조문도 article_no 전체(의N 포함)로 파싱된다."""
        from app.retrieval.refs import parse_ref
