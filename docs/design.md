...
| Ollama Cloud (gpt-oss:20b) | RARR 주장 분해·CQGen·agreement 판정 — 내부 기계적 판단 (결정 N) |
...
**복잡 (full research, `_search_complex`)**
```
1. CQGen: 주장별 검증 질문 생성 (gpt-oss:20b, RARR_QUESTIONS_PER_CLAIM cap)
...
질의 -> 세무사 수동 토글 (simple | complex) — classify()는 패스스루 (결정 A/A2)
     -> 벡터 top-1 코사인 경량 체크(_promotion_score, app/api/chat.py)가
        임계값(fallback_score_threshold) 미달이면
        -> 자동으로 복잡 모드로 승격 (should_promote, app/router/mode_classifier.py)
```

승격 판정용 top 점수는 `_retrieve_simple`(리랭크+그래프확장까지 도는 풀 파이프라인)이
아니라 `_promotion_score`(임베딩 1회 + 벡터검색 top-1만)에서 나온다 — 승격 신호 하나
읽으려고 풀 파이프라인을 돌리고 버리던 낭비를 제거했다. `_retrieve_simple` 자체는
claim별 실제 research(`app/rarr/research.py`)에서 계속 쓰인다.
...
2 DECOMPOSE    초안 → 원자 주장 목록 (gpt-oss:20b, decontextualized)
...
4 AGREEMENT    (주장, 근거) 일치 판정 (gpt-oss:20b) + cited_refs ⊆ supporting 불변식 위반 시 강등
...
추론:       Gemini(gemini-2.5-flash, draft/edit/reason) + Ollama Cloud(gpt-oss:20b, aux) — 역할 기반 분리(결정 N)
