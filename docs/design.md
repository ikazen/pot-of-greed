...
[세무사 클라이언트]
        | HTTPS
        v
[OCI: API 서버 (FastAPI, async)]
        |
        +--> PostgreSQL + pgvector   : 벡터검색 + 키워드(tsvector) + 메타데이터 + 로그
        +--> Neo4j                   : 인용/준용 그래프 + 판례 유효성 + 벡터인덱스(옵션)
        |
        +--(쿼리 임베딩)--> [온프레미스: Ollama embedding]
        +--(LLM 추론)----> [Ollama Cloud]
```
...
| 저장소 | 역할 |
|---|---|
| PostgreSQL + pgvector | 벡터 의미검색, 키워드 정확매칭(tsvector/BM25), 조문/판례 원문, 메타데이터, 사용자/채팅 로그 |
| Neo4j | 조문↔판례 인용, 조문↔조문 준용, 판례변경(OVERRULED_BY), 조문 개정이력 그래프 탐색 |
| 온프레미스 Ollama | 임베딩 생성 (인덱싱 배치 + 런타임 쿼리 임베딩) |
| Ollama Cloud | 복잡 추론, 쿼리 분해, 충분성 평가, 근거 검증 |
...
3. 경량 리랭킹 (top-30 -> top-5)
4. Neo4j 1홉 확장 (직접 인용 판례만) + validity_flag 부착
5. 단일 추론 패스 (Ollama Cloud)
6. 출처 부착
   ※ HyDE/분해/자기교정/근거검증 생략
```

### 5.2 복잡 모드 (10~20초 허용)

```
1. 쿼리 분해 (복합 질의 -> 쟁점별 하위질의)
2. 하위질의별 도구 라우팅 [하이브리드 검색 / Neo4j 탐색]
3. HyDE + 하이브리드 검색(넓게) + 강한 리랭킹
4. Neo4j 2홉 확장 (인용 판례, 준용 조문, OVERRULED_BY, 개정이력)
5. 충분성 평가 루프 (부족 -> 쿼리 재작성 후 재검색, 최대 N회)
6. 강한 추론 패스 (Ollama Cloud)
7. 근거 검증 게이트 (생성된 각 주장이 검색 근거에 실재하는지 LLM 교차확인 = 환각 차단)
8. 출처 부착
```
...
```
질의 -> 경량 분류기(작은 LLM, 빠름) -> {단순 | 복잡}

단순 트리거: 단일 쟁점, 특정 조문/판례 직접 조회, 정의성 질문
복잡 트리거: 복수 쟁점, 사실관계 적용, 판례 종합, 절세 판단

Fallback 승격: 단순 모드 검색 top 점수가 임계값 미달이거나
              분류가 애매하면 -> 자동으로 복잡 모드로 승격
```
...
```
API 서버:   FastAPI (async) — Ollama 양쪽 호출이 I/O 바운드
DB:         PostgreSQL 16+ / pgvector (hnsw), Neo4j 5.x
임베딩:     Ollama, bge-m3 (1024차원, 한국어 강함) — 온프레미스
리랭커:     bge-reranker-v2-m3 — 온프레미스
추론:       Ollama Cloud (강한 추론 모델)
...
app/
  ...
  api/
    chat.py                # POST /chat — 질의 진입점
  router/
    mode_classifier.py     # 단순/복잡 라우팅 + fallback 판정
  retrieval/
    ...
    graph_expand.py        # Neo4j 1홉/2홉 확장
    hyde.py                # 복잡 모드 가상답변 생성
  agent/
    decompose.py           # 쿼리 분해
    tool_router.py         # 하위질의 -> 도구 선택
    sufficiency.py         # 충분성 평가 루프
    grounding_check.py     # 근거 검증 게이트
  reasoning/
    llm_client.py          # Ollama Cloud 클라이언트
    answer_builder.py      # 출처 부착, 유효성 경고 병기
  db/
    pg.py
    neo4j.py
```
